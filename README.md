...
| 包 | 职责 | 备注 |
| --- | --- | --- |
| `result` | 统一响应 `Result{code,message,data,success}` 与分页 `PageData{total,headNodeTotal,pageIndex,pageSize,list}` | 平台统一响应信封 |
| `apperr` | HTTP 状态码语义错误（未登录 401 / 无权限 403 / 404 / 409 / 503 / 422 / 400 / 429 / 500）+ 业务失败 `Business` | 异常兜底语义 |
| `authn` | 登录态抽象 `SessionProvider` / `Manager` | satoken 与 jwtauth 共用 |
| `config` | 环境变量 + YAML 配置加载，支持 `${ENV:default}` 占位符；`auth.mode=satoken\|jwt` | — |
...
| `jwtauth` | **JWT（HS256）无状态登录态**：无 Redis 场景；与 satoken 会话不互通；Logout 默认 no-op | `auth.mode=jwt` |
| `middleware` | 认证中间件、Trace 中间件、统一异常/恢复、CORS | 会话拦截；`AuthConfig.Provider` 可接 satoken/jwtauth |
